...
- [ ] After AUTH-SIGNUP-01 succeeds: try to sign up _again_ with the same email.
...
- [ ] **Expect** `/billing` status panel shows a warning Alert: _"Cancellation scheduled for {date}. Resume your subscription before then to keep access."_ with a **Resume** button on the right.
...
- [ ] UI shows error Alert: _"Google returned an error: **access_denied**. Try connecting again."_
...
- [ ] In the Inngest dev UI, the `gmail-backfill` run shows 3 steps: `gmail-backfill` succeeded, `gmail-backfill-process-opportunities` succeeded, `gmail-start-watch` succeeded.
...
- [ ] Connect Gmail at `/settings/email`. Inngest UI's `gmail-backfill` run shows `gmail-backfill` ✓, `gmail-backfill-process-opportunities` ✓, then `gmail-start-watch` ✓. DB `watchExpiresAt` is ~7 days in the future.
...
### OPP-01: Backfill materializes quote requests into opportunities

- [ ] Set `OPENAI_API_KEY` in `apps/api/.env`, restart API, and keep Inngest dev running.
- [ ] Connect a Gmail or Microsoft mailbox that contains at least one real Dutch quote-request email.
- [ ] In Inngest dev UI, wait for the provider backfill run to complete. The processing pass shows as `*-process-opportunities-batch-0`, `-batch-1`, …; each batch is its own resumable step.
- [ ] DB check: `SELECT count(*) FROM "Opportunity" WHERE "emailAccountId" = '<connected-account-id>';` returns at least 1 for a mailbox with quote requests.
- [ ] DB check: every created row has `status = 'new'`, non-empty `requestType`, `urgency` (one of `emergency` / `high` / `normal` / `low`), `aiProvider` shaped like `<provider>/<model>` (e.g. `openai/gpt-4o`), populated `classifiedAiCallId` + `extractedAiCallId`, and a unique `rawMessageId`.
- [ ] DB check: the linked `RawMessage` rows have `isQuoteRequest = true` and `classifiedAt IS NOT NULL`.
- [ ] DB check: `SELECT id, provider, model FROM "AICall" WHERE id IN (<classifiedAiCallId>, <extractedAiCallId>);` returns both rows with `purpose IN ('classifier', 'extractor')`.

### OPP-02: Non-quote emails are classified but do not create opportunities

- [ ] Use a connected mailbox with an obvious non-quote email, or insert a fixture-style `RawMessage` for a newsletter/calendar/update.
- [ ] Invoke the matching provider backfill/delta function, or call the opportunities processor through the Inngest step by re-running the account sync.
- [ ] **Expect** no `Opportunity` row for that `RawMessage`.
- [ ] DB check: the `RawMessage` row has `isQuoteRequest = false` and `classifiedAt IS NOT NULL`.

### OPP-03: Opportunity creation is idempotent

- [ ] After OPP-01 succeeds, invoke the same provider backfill/delta function again for the same `EmailAccount`.
- [ ] **Expect** no duplicate opportunities.
- [ ] DB check: `SELECT "rawMessageId", count(*) FROM "Opportunity" GROUP BY "rawMessageId" HAVING count(*) > 1;` returns zero rows.
- [ ] Inngest output may show zero new opportunities or skipped duplicates; both are acceptable as long as the DB remains unique.

### OPP-04: Extractor failure leaves positives retryable

- [ ] Temporarily force the extractor to fail locally, for example by setting an invalid `OPENAI_MODEL_EXTRACTOR` while keeping classifier configuration valid.
- [ ] Run the processing step against a mailbox with a quote request.
- [ ] **Expect** the processing step logs `opportunity.pipeline.raw_message_failed`.
- [ ] DB check: the raw message does **not** get `classifiedAt` set and no `Opportunity` row is created for it.
- [ ] Restore the extractor config and rerun the processor. The same raw message should now create an opportunity.

### OPP-05: Status transitions are server-enforced

- [ ] With an existing opportunity, `PATCH /api/opportunities/<id>/status` as an authenticated active-org member with body `{ "status": "cold" }`.
- [ ] **Expect** HTTP 200 and response `status = "cold"` when the current status is `new`.
- [ ] Try to move a terminal opportunity back to `new`, for example `won -> new`.
- [ ] **Expect** HTTP 400 with message `Cannot transition opportunity from won to new.`
- [ ] Try the same endpoint from another tenant. **Expect** 404, not another org's opportunity data.

### OPP-06: List endpoint is org-scoped and paginates by cursor

- [ ] Seed Opportunities in two organizations (Org A, Org B).
- [ ] Sign in as a user whose `currentOrganizationId` points at Org A; `GET /api/opportunities`.
- [ ] **Expect** every returned `organizationId` matches Org A. None of Org B's rows leak.
- [ ] Switch the user's active organization to Org B (POST `/api/me/organization`), retry the GET.
- [ ] **Expect** every returned `organizationId` now matches Org B.
- [ ] Pagination: `GET /api/opportunities?limit=2` against an org with 5+ opportunities returns `opportunities.length === 2` and a non-null `nextCursor`.
- [ ] `GET /api/opportunities?cursor=<nextCursor>&limit=2` returns the next 2 rows in `createdAt DESC, id DESC` order with no overlap.
- [ ] Walking the cursor chain to the last page returns `nextCursor: null`.
- [ ] `GET /api/opportunities?limit=999` is clamped server-side; response page size ≤ 100.
- [ ] `GET /api/opportunities?cursor=garbage` returns the first page (decoder is tolerant; treats unparseable cursors as "no cursor").
